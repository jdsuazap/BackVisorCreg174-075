using Core.Entities.SQLContext;
using Core.Interfaces.SQLContext;
using Infrastructure.Data;

namespace Infrastructure.Repositories.SQLContext
{
    public class TipoPersonaRepository : BaseRepository<TipoPersona>, ITipoPersonaRepository
    {
        public TipoPersonaRepository(DbSQLContext context) : base(context)
        {
        }

        public async Task<TipoPersona> CreateEntity(TipoPersona entity)
        {
            await Add(entity);
            return entity;
        }

        public async Task<bool> DeleteEntity(TipoPersona entity)
        {
            var entidad = await GetById(entity.Id);

            entidad.Estado = false;
            entidad.CodUserUpdate = entity.CodUserUpdate;
            entidad.FechaRegistroUpdate = entity.FechaRegistroUpdate;

            await Update(entidad);

            return true;
        }

        public async Task<List<TipoPersona>> GetEntities()
        {
            return await GetAll();
        }

        public async Task<TipoPersona> GetEntity(int idEntity)
        {
            return await GetById(idEntity);
        }

        public async Task<TipoPersona> UpdateEntity(TipoPersona entity)
        {
            var entidad = await GetById(entity.Id);

            entidad.Descripcion = entity.Descripcion;
            entidad.Estado = entity.Estado;
            entidad.CodUserUpdate = entity.CodUserUpdate;
            entidad.FechaRegistroUpdate = entity.FechaRegistroUpdate;

            await Update(entidad);

            return entidad;
        }
    }
}
