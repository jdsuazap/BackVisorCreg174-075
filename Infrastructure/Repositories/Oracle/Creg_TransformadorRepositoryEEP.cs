    ...
    public class Creg_TransformadorRepositoryEEP : BaseRepositoryDapperOracleSpard<Transfor>, ICreg_TransformadorRepository
    {
        ...
        public async Task<Transfor> GetCargabilidadTransformador(int ciudad, string codtrafo)
        {
            try
            {
                ...
                if (ciudad == (int)EmpresaEnum.Pereira)
                {
                    sql = @"
                        WITH DATA AS (
                            SELECT
                ...
                else
                {
                    sql = $@"
                        ...
                        TRANSFORMERS_UNION AS (
                            ...
                            SELECT CODE, TRFTYPE
                            FROM SPARD.TRANSFOR{SpardCTG.DBLINK}
                            WHERE DELETED = 0
                        ),
                        TRANSFORMERS_WITH_KVA AS (
                            SELECT 
                                CODE,
                                TRFTYPE,
                                CASE
                        ...
                        FROM AGG A
                        INNER JOIN TRANSFORMERS_WITH_KVA T 
                            ON T.CODE = A.CODE_T
                        WHERE A.CODE_T LIKE '%" + codtrafo + "%'";
                }
                ...
                string sql = $@"
                            ...
                            UNION ALL 
                                SELECT 
                                    ...
                                    A.VSEC1,
                                    A.VSEC2
                                FROM SPARD.TRANSFOR{SpardCTG.DBLINK} A                                
                                WHERE 
                                    A.CODE = '{codigo}'
                                    AND A.DELETED = 0";
            ...
            string sql = $@"SELECT 
                        ...
                        UNION ALL
                            SELECT 
                                ...
                                ROUND(FEEDERS.CME_12,3) AS ENER_HOR_DISPO_FV_CIRC
                            FROM SPARD.TRANSFOR{SpardCTG.DBLINK},
                                SPARD.TRFTYPES{SpardCTG.DBLINK},
                                SPARD.FEEDERS{SpardCTG.DBLINK},
                                SPARD.SRCBUSES{SpardCTG.DBLINK}
                            WHERE 
                                TRANSFOR.TRFTYPE = TRFTYPES.CODE 
                                AND TRANSFOR.FPARENT = FEEDERS.CODE 
                                AND FEEDERS.SOURCEBUS = SRCBUSES.CODE 
